            ...
            for (int j = 0; j < tab[i].length; j++) {
                ...
                if (j == 5) {
                    ...
                    if (nomT.length() >= 3) {
                        ...
                        //On prent le 3 lettre nom+la premiere lettre du prenom+l'année de naissance pour créer le username
                        String userName = nomT.substring(0, 2) + tab[i][1].toLowerCase().substring(0, 1) + tab[i][3];
                        //On donne l'username obtenu a la position
                        tab[i][5] = userName;
                    }
...
//              On stocke dans notre tableau les données de notre fonction saisieEleve
    ...
    static void rechercheEtudiant() {
        ...
        if (result > -1) {
            ...
            //On crée un nouveau tableau qui va stocker l
            String[][] affichResult = new String[compt][6];
            int compt2 = 0;
            //On copie les resultat dans le nouveau tableau
            for (int i = 0; i < tableauEleve.length; i++) {

                if (tableauEleve[i][1].equalsIgnoreCase(recP) && tableauEleve[i][2].equalsIgnoreCase(recN)) {

                    for (int j = 0; j < affichResult[0].length; j++) {

                        affichResult[compt2][j] = tableauEleve[i][j];
            ...
            for (int i = 0; i < affichResult.length; i++) {
                for (int j = 0; j < affichResult[i].length; j++) {

                    System.out.printf(format,affichResult[i][j]);
